...
def train_rltools(config, use_python_environment=True, verbose=False):
    custom_environment = {
    ...
    if use_python_environment:
        ppo = PPO(env_factory, enable_evaluation=True, interface_name=interface_name, force_recompile=not "RL_TOOLS_SKIP_FORCE_RECOMPILE" in os.environ, verbose=verbose, **kwargs)
    else:
    ...
    for step_i in range(config["n_steps"]):
        if step_i % config["evaluation_interval"] == 0:
            current_returns = evaluate_policy(lambda observation: state.action(observation), config, env_factory, render=config["render"])
